...
agent.py 主循环 ──执行──▶ tools.py 分派 ──▶ xor_world.py（改 grid / 渲染）
   ...
   ▼
output/<运行ID>/state.json   # 每次运行独立文件夹（秒精度时间戳，同秒冲突加字母）
   │ ③ SSE 推送（/events：思考 delta 与快照到达即推；web 3s 低频轮询兜底）
   ▼
web/index.html（画布 / 参数示意图 / 决策过程：思考+点击）
```
...
主循环（流式）：AI 调用工具 → 执行 → 结果文本回填 messages → 循环，直到 AI 不再调用工具或触达终止条件。每次工具调用后写一份快照到 `output/<运行ID>/state.json`（每次运行独立文件夹，历史运行不被覆盖）。模型可能一次请求返回多个工具调用（批量并行，实测一轮 16~31 个，或者一轮填满 63 格），点击按轮次（round）记录，供可视化按轮分组。
...
**架构一句话**：agent 每走一步把状态落盘到 `output/<运行ID>/state.json`（JSON，每次运行独立文件夹）并广播 SSE 事件（`/events` 长连接），`web/index.html` 用 EventSource 订阅实时渲染；中间只有一个标准库 HTTP 服务，无框架、无 CDN、无 WebSocket。
...
- 路径白名单：`/` 或 `/index.html` 改写为 `/web/index.html`；只放行 `/web/` 与 `/output/` 前缀的文件，以 `/` 结尾的目录请求一律 404（不列目录、不枚举文件），其余路径一律 404——所以 `/output/state.json`、`/output/reasoning_live.json` 可直接 fetch（这两个固定 URL 由服务端映射到**当前运行文件夹** `output/<运行ID>/` 下的同名文件，见下），而 `.env`、`*.py` 等根目录文件不可达。
- 运行产物按运行分文件夹（2026-09-01 起）：`init_run_dir()` 在启动时（`main()` 与 `sim_agent.py` 回放均走此函数）创建 `output/<时间戳>/`，秒精度 `%Y%m%d_%H%M%S`（与日志命名 `xor_agent_*.log` 同款），**同秒冲突自动加字母 a/b/…**（`os.mkdir` 原子创建 + `FileExistsError` 后 `_next_letter` 递增重试，两个进程同秒启动也各拿唯一文件夹）。`state.json` 与渲染图 `state.png`（每次工具调用后覆盖写本运行文件夹内）及流式思考中间文件 `reasoning_live.json` 都在该文件夹内——历史运行不被覆盖；`output/` 根目录不落任何实时文件，只含各次运行的文件夹。
...
- `xor_world.snapshot(out_dir)`：**每次工具调用后**写 `output/<运行ID>/state.json`（含 grid、seed_index、seed_value、clicks（每条含 reasoning、round）、status、final_reason）与 `state.png`（同一画布状态的 512×512 灰度渲染图，numpy `render()` + PIL 存盘，随快照覆盖写、运行结束即最终画布，供后台存档/离线查看）。前端不读此图——按 grid 在 canvas 复刻 `(dx^dy)&a` 渲染（与原始作品 Interactive Drawing XOR.html 一致）。启动时先做一次初始快照。
...
- 流式实时思考：API 调用用 `stream=True`，思考增量（reasoning_content）实时打印到控制台、原子写当前运行文件夹内 `reasoning_live.json`（`{round, reasoning}`，web 兜底轮询用）并 SSE 广播 `reasoning` 事件（`{round, reasoning}` 全文，前端即时显示）；一轮结束后由下一轮覆盖，运行结束删除。流式仅改变传输方式，不改变 token 计费，无额外成本。
...
- 智能跟随滚动（2026-08-25，方向感知 + 按钮兜底，见 context-log/2026-08-25_滚动跟随修复.md）：向上滚动且距底 ≥ 40px（PAUSE_DIST）→ 暂停跟随，内容照常追加不打扰；向下滚动且距底 < 150px（RESUME_DIST）→ 自动恢复跟随（方向感知，避免"内容持续追加、底部移动"时差一点够不到底；单一阈值放大会导致翻历史时被误拉回，故暂停/恢复阈值分离）；暂停时时间线右下角浮现「↓ 回到底部」按钮，点击必回底并恢复跟随。
- 渲染三块：当前画布（`<canvas id="canvas-main" width="512" height="512">`，前端按 grid 复刻 `(dx^dy)&a` 逐像素渲染——每区 64×64，负值 mask 写 `-a-1`（numpy 语义，JS 的 `~a` 是 32 位补码不能直接用）；参数示意图（canvas 2D，每格灰度 = 该区白像素占比——只取决于参数二进制 1 个数 k：ratio=(64−2^(6−k))/64，负值取补（128 值暴力遍历校验一致）；文字黑/白随底明暗、种子格边框随之切换；原「热力图」色相隐喻已弃用，2026-08-28 定稿，见 context-log/2026-08-28_参数格显示方案测试.md）；**AI 决策过程：思考 + 点击**（一张卡，按 round 分组、最新 20 组；进行中轮思考显示当前累积全文并标「思考中…」、历史轮显示完整思考，一组下面跟该批格子，reasoning 经 `esc()` 转义防 HTML 注入）。
