...
3. 图案可文字精确描述（a 的二进制第 k 位 = 块大小 2^k 的棋盘格；多位置位 = 白格取并集），AI 不用看图也能推理。
4. 单一指标有作弊解（全填 a=1 等单一位值即黑白各半满分）：v1 目的是跑通闭环+可视化，审美要等指标库扩展。
...
## 相关文件
- TECH_DESIGN.md - 技术设计（结论存档，4 顶级章节各 ≤4 子标题）；DEPENDENCIES.md - 依赖与许可证
...
### 2026-08-05
- 拆分 DESIGN.md，技术设计部分放进 TECH_DESIGN.md：Google Labs 2026-03 的 DESIGN.md 规范已把 DESIGN.md 定为视觉设计系统文件（tokens+规则），为避免歧义，技术设计改用 TECH_DESIGN.md；结构重排为"背景与决策→整体架构→关键机制→实验设计"（自顶向下，同级标题 ≤4）
- 依赖与许可证从技术设计文档拆出为 DEPENDENCIES.md
- 修正图案语义事实错误：a=63 几乎全白只剩主对角线（不是最密）；作弊解是单一位值 a=1（不是 ±63）
